...
export function listPaintingBriefs() {
  return data.paintings.map((p) => ({
    ...
    artist: { name: p.artist.name, dates: p.artist.dates },
    series: p.series,
...
export function getArtistOf(painting: Painting): ArtistInfo {
...
export function listArtists(): ArtistInfo[] {
  const seen = new Map<string, ArtistInfo>();
  for (const p of data.paintings) {
    if (!seen.has(p.artist.name)) seen.set(p.artist.name, p.artist);
  }
